        ...
        <DialogActions>
          ...
          <Button onClick={async ()=> {
            if(parseInt(dishAmount) < 0 ) {
              return swal("Thông báo", "Số lượng món không hợp lệ, Vui lòng thử lại ", "error")
            } 
            if(parseInt(dishPrice) < 0 ) {
              return swal("Thông báo", "Giá món không hợp lệ, Vui lòng thử lại ", "error")
            } 
            else {
              const result= await add_custom_dish(orderId, dishName, dishDescription, dishPrice, dishAmount)
