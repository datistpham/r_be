    ...
    try {
        ...
        if(rows.length > 0) {
            if(parseInt(rows[0]?.role) === 3) {
                return res.status(200).json({auth: true, id_user: rows[0].id_user, firstName: rows[0].first_name, lastName: rows[0].last_name, email: rows[0].email, isAdmin: true})
            }
            else if(parseInt(rows[0]?.role) === 2) {
                return res.status(200).json({auth: true, id_user: rows[0].id_user, firstName: rows[0].first_name, lastName: rows[0].last_name, email: rows[0].email, isEmployee: true})
