...
const custom_book_dish= expressAsyncHandler(async (req, res)=> {
    try {
        const dish_id= v4()
        const [rows]= await connection.execute("INSERT INTO dish(dish_id, dish_name, dish_description, dish_price, mode) VALUES(?, ?, ?, ?, ?)", [dish_id, req.body.dishName, req.body.dishDescription, req.body.dishPrice, 1])
